...
public class Board {
	...
	public Cell getCellAt(Location loc) {
		...
		if (loc.row <0 || loc.col < 0) return null;
	...
	public boolean isFinished() {
		for(int i = 0; i < cells.length; ++i) {
			if (cells[i] != null && cells[i].Index != i)
				return false;
		}
	...
	private static Cell[] divideUpImage(int[] image, int width, int dimension) {
		...
		for (int y = 0; y < dimension; y++) {
			for (int x = 0; x < dimension; x++) {
				...
				result[index] = carveOutCell(image,width,dimension,x,y,index);
			}
		}
	 ...
	 * @return
	 */
	private static Cell carveOutCell(int[] image, int width, int dimension, int cx, int cy, int index) {
		int cellWidth = width / dimension;
		...
		return new Cell(part, cellWidth, index);
	}
}
