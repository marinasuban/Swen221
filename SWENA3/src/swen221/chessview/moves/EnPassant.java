...
public class EnPassant extends SinglePieceTake {
	...
	public boolean isValid(Board board) {
		Piece piece = board.pieceAt(getTakenPosition());
		//if not a valid position return false
		if(!checkValidPositions()) {
			return false;
		}
		//if piece taken is not valid return false
		if(!checkValidPieceTaken(board)) {
			return false;
		}
		//if pawn piece was not double step return false
		if(!((Pawn)piece).wasDoubleStep()) {
			return false;
			}
		return true;
	}
